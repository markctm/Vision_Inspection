    ...
    def OCR_Search_FW_Version(self,string,restest,img1,imgref):
        ...
        if str(string) in text2:
            ...
            if (self.Count_Serial_TestTime_Occurence(SerialNumber) > self.Get_Retest_Times_Before_Fail()):
                ...
                cv2.putText(img1, "MES REJECTION"+ str(self.Count_Serial_TestTime_Occurence(SerialNumber)), (50, 680), fonte, 1.0, (0,0,255), 1.5, cv2.LINE_AA)
            else:
                cv2.putText(img1, "RETEST NUMBER: "+ str(self.Count_Serial_TestTime_Occurence(SerialNumber)), (50, 680), fonte, 1.0, (0,0,255), 1.5, cv2.LINE_AA) 
